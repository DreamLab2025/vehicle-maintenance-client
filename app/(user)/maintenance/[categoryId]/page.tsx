...
function MaintenanceCategoryPageContent() {
  ...
  const matchedCategoryFromQuery = useMemo(() => {
    if (categoryNameFromQuery && categoriesData?.length > 0) {
      return categoriesData?.find((c) => c.name === categoryNameFromQuery) || null;
    }
    return null;
  }, [categoryNameFromQuery, categoriesData]);
  ...
  } = useProductsByCategory(effectiveCategoryId || "", step === "products" && !!effectiveCategoryId, {
  ...
  const selectedCategory = useMemo(() => {
    ...
    return categoriesData?.find((c) => c.id === effectiveCategoryId) || null;
  }, [effectiveCategoryId, categoriesData]);
  ...
  const handleAddItem = useCallback(() => {
    ...
    if (!isCustomProduct) {
      ...
      const isAlreadyAdded = selectedItems.some((item) => !item.isCustom && item.productId === currentProductId);
