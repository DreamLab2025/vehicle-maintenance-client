...
import { useCreateProduct, useDeleteProduct, useProductsByCategory, useUpdateProduct } from "@/hooks/usePartProducts";
import { usePartCategories } from "@/hooks/usePartCategories";

import ProductsFilters, { ProductStatusFilter } from "./components/ProductsFilters";
...
export default function ProductsPage() {
  /** ===== Categories ===== */
  const catParams = useMemo(() => ({ PageNumber: 1, PageSize: 200, IsDescending: false }), []);
  ...
  const [userSelectedCategoryId, setUserSelectedCategoryId] = useState<string>("");
  ...
  const { products, isLoading, isFetching, isError, refetch } = useProductsByCategory(
    selectedCategoryId,
    productsEnabled,
  );
  ...
  const filtered = useMemo(() => {
    ...
    const base = (products ?? []).filter((p) => {
      ...
      // const matchesStatus =
      //   statusFilter === "All" ? true : p.isActive;
  ...
  const deletingId: string | null = deleteMutation.isPending ? (deleteMutation.variables?.id ?? null) : null;
  ...
  return (
    <div className="container mx-auto py-8 space-y-8 max-w-7xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-5">
        <div>
          <h1 className="text-3xl tracking-tight text-slate-900">Quản lý Sản phẩm Phụ tùng</h1>
        </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              <option value="" disabled>
                {catsLoading || catsFetching ? "Đang tải danh mục..." : "Chọn danh mục"}
              </option>
            ...
            {showEmptyCategories && <div className="text-xs text-slate-500">Chưa có danh mục Active để chọn.</div>}
          </div>
      ...
      <div className="space-y-4 px-5">
        <ProductsToolbar
          ...
          onRefresh={() => refetch()}
        />
        <ProductsFilters status={statusFilter} onStatusChange={setStatusFilter} />
      </div>
      ...
      <div className="min-h-[420px] px-5">
        {!selectedCategoryId ? (
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-slate-100 shadow-sm">
            ...
            <p className="text-slate-500 font-medium">Vui lòng chọn danh mục để xem sản phẩm</p>
          </div>
        ) : isError ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-slate-100 shadow-sm">
            ...
            <p className="text-slate-500 font-medium">Không có sản phẩm nào phù hợp</p>
            <Button
              variant="link"
          ...
          <div className="space-y-6">
            <ProductsTable data={filtered} onEdit={handleOpenEdit} onDelete={handleDelete} isDeletingId={deletingId} />
          </div>
        )}
      </div>
