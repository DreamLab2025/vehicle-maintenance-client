...
export default function ProductsDialog(props: {
  ...
  const categoryDisabled = isSubmitting || catsLoading || catsFetching || (lockCategory && !!defaultCategoryId);

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[720px] bg-white">
        <DialogHeader>
          <DialogTitle className="text-slate-900">{isEditMode ? "Cập nhật sản phẩm" : "Thêm sản phẩm mới"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm((s) => ({ ...s, partCategoryId: e.target.value }))}
          ...
          <div>
            <label className="text-xs text-slate-600">Giá tham khảo (VND)</label>
            <input
              type="number"
