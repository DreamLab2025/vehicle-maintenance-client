...
function getNotifIcon(notification: Notification) {
  ...
  if (notification.type === "odometer_update") return { Icon: Gauge, bg: "#f0fdf4", color: "#16a34a" };
  if (notification.type === "maintenance") return { Icon: Wrench, bg: "#eff6ff", color: "#3b82f6" };
  if (notification.type === "promotion") return { Icon: Megaphone, bg: "#fdf4ff", color: "#a855f7" };
...
function NotificationRow({
  ...
  const levelConfig = hasLevel ? getReminderLevelConfig(notification.level!) : null;
        ...
        <div className="flex items-start justify-between gap-3">
          <p
            className={`text-sm leading-snug ${
              !notification.isRead ? "font-semibold text-neutral-900" : "font-medium text-neutral-500"
            }`}
          >
            {notification.title}
          </p>
          <span className="flex-shrink-0 text-xs text-neutral-400 mt-0.5">{timeAgo(notification.createdAt)}</span>
        </div>
        ...
        >
          ...
          <div className="p-5">
            ...
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-base font-bold text-neutral-900 leading-tight">{notification.title}</p>
                <p className="text-xs text-neutral-400 mt-1">{formatFullDate(notification.createdAt)}</p>
              </div>
            ...
            {/* Full message */}
            <p className="text-sm text-neutral-600 leading-relaxed mt-4">{notification.message}</p>
    ...
    <div className="min-h-screen bg-white pb-28 overflow-y-auto scrollbar-hide">
      ...
      <div className="flex items-center gap-6 px-6 pt-3 pb-1">
        {[
          ...
          <button key={tab.key} type="button" onClick={() => setFilter(tab.key)} className="relative pb-2">
            <span
              className={`flex items-center gap-1.5 text-sm transition-colors ${
                filter === tab.key ? "font-semibold text-neutral-900" : "font-medium text-neutral-400"
              }`}
            >
              {tab.label}
              {tab.count > 0 && (
                <span
                  className={`min-w-[18px] h-[18px] px-1 flex items-center justify-center rounded-full text-[10px] font-bold ${
                    filter === tab.key ? "bg-red-500 text-white" : "bg-neutral-200 text-neutral-500"
                  }`}
                >
                  {tab.count}
                </span>
      ...
      <div className="pt-2">
        {isLoading ? (
              ...
              <div className="bg-white mx-4 rounded-2xl overflow-hidden shadow-sm shadow-neutral-200/40">
                {group.items.map((notification, i) => (
                  <div key={notification.id}>
                    ...
                    <NotificationRow notification={notification} onSelect={handleSelect} index={i} />
                  </div>
                ))}
              </div>
            </div>
          ...
          >
            ...
            <p className="text-sm font-medium text-neutral-400">{"Không có thông báo"}</p>
          </motion.div>
        )}
      </div>
